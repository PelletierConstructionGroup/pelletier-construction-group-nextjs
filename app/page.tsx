  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', minHeight: "100vh", backgroundColor: "white", color: "black" }}>
      ...
      <ImageList
        ...
        cols={isXs ? 1 : isSm ? 2 : 3} // Responsive column count based on screen size
        rowHeight={330}
      >
        {itemData.map((item) => (
          <ImageListItem key={item.img} sx={{ marginLeft: 1, marginRight: 1 }}>
            <Image
              src={`${item.img}`}
              alt={item.title}
              layout="responsive" // Responsive layout for images
              width={100} // Set a baseline width for aspect ratio
              height={100} // Set a baseline height for aspect ratio
              style={{
                width: '100%',
                height: 'auto',
                objectFit: 'cover',
